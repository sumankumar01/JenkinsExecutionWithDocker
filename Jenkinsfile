	...
	stage('Build Jar') {
      ...
      stage('Run Test') {
         steps{
                  ...
                  sh "docker run --rm -e SELENIUM_HUB=${seleniumHub} -e BROWSER=chrome -e MODULE=CucumberOptions.TestRunner -v ${WORKSPACE}/:/usr/share/suman/ --network ${network} vagrant/containertest"
